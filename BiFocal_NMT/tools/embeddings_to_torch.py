...
def get_vocabs(dict_path):
    ...
    for side in ['src', 'qry', 'tgt']:
        ...
        vocs.append(vocab)
    enc_vocab, match_vocab, dec_vocab = vocs

    logger.info("From: %s" % dict_path)
    logger.info("\t* source vocab: %d words" % len(enc_vocab))
    logger.info("\t* match vocab: %d words" % len(match_vocab))
    logger.info("\t* target vocab: %d words" % len(dec_vocab))

    return enc_vocab, match_vocab, dec_vocab
...
def main():
    ...
    parser.add_argument('-emb_file_both', required=False,
                        help="loads Embeddings for both source, query and target "
                             "from this file.")
    parser.add_argument('-emb_file_enc', required=False,
                        help="source Embeddings from this file")
    parser.add_argument('-emb_file_match', required=False,
                        help="Query Embeddings from this file")
    ...
    enc_vocab, match_vocab, dec_vocab = get_vocabs(opt.dict_file)
    ...
    if opt.emb_file_both is not None:
        if opt.emb_file_enc is not None:
            raise ValueError("If --emb_file_both is passed in, you should not"
                             "set --emb_file_enc.")
        if opt.emb_file_match is not None:
            raise ValueError("If --emb_file_both is passed in, you should not"
                             "set --emb_file_match.")
        if opt.emb_file_dec is not None:
            raise ValueError("If --emb_file_both is passed in, you should not"
                             "set --emb_file_dec.")
        set_of_src_qry_tgt_vocab = \
            set(enc_vocab.stoi.keys()) | set(match_vocab.stoi.keys()) | set(dec_vocab.stoi.keys())
        logger.info("Reading encoder, matcher and decoder embeddings from {}".format(
            opt.emb_file_both))
        src_vectors, total_vec_count = \
            read_embeddings(opt.emb_file_both, skip_lines,
                            set_of_src_qry_tgt_vocab)
        qry_vectors = src_vectors
        ...
        if opt.emb_file_enc is None:
            raise ValueError("If --emb_file_enc not provided. Please specify "
                             ...
                             "--emb_file_both")
        if opt.emb_file_match is None:
            raise ValueError("If --emb_file_match not provided. Please specify "
                             "the file with matcher embeddings, or pass in "
                             "--emb_file_both")
        ...
        logger.info("Reading matcher embeddings from {}".format(
            opt.emb_file_match))
        qry_vectors, total_vec_count = read_embeddings(
            opt.emb_file_match, skip_lines,
            filter_set=match_vocab.stoi
        )
        logger.info("\tFound {} total vectors in file.".format(
            total_vec_count))
    ...
    logger.info("\t* match: %d match, %d missing, (%.2f%%)"
                % calc_vocab_load_stats(match_vocab, qry_vectors))
    logger.info("\t* dec: %d match, %d missing, (%.2f%%)"
                % calc_vocab_load_stats(dec_vocab, src_vectors))

    # Write to file
    enc_output_file = opt.output_file + ".enc.pt"
    match_output_file = opt.output_file + ".match.pt"
    dec_output_file = opt.output_file + ".dec.pt"
    logger.info("\nSaving embedding as:\n\t* enc: %s\n\t* match: %s\n\t* dec: %s"
                % (enc_output_file, match_output_file, dec_output_file))
    torch.save(
    ...
    torch.save(
        convert_to_torch_tensor(qry_vectors, match_vocab),
        match_output_file
    )
    torch.save(
