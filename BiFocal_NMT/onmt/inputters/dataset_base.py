...
def _dynamic_dict(example, src_field, qry_field, tgt_field):
    ...
    qry = qry_field.tokenize(example["qry"])
    unk = src_field.unk_token
    pad = src_field.pad_token

    # make a small vocab containing just the tokens in the source sequence
    src_ex_vocab = Vocab(Counter(src), specials=[unk, pad])
    unk_idx_src = src_ex_vocab.stoi[unk]
    # Map source tokens to indices in the dynamic dict.
    src_map = torch.LongTensor([src_ex_vocab.stoi[w] for w in src])
    example["src_map"] = src_map
    example["src_ex_vocab"] = src_ex_vocab

    # make a small vocab containing just the tokens in the query sequence
    qry_ex_vocab = Vocab(Counter(qry), specials=[unk, pad])
    unk_idx_qry = qry_ex_vocab.stoi[unk]
    # Map query tokens to indices in the dynamic dict.
    qry_map = torch.LongTensor([qry_ex_vocab.stoi[w] for w in qry])
    example["qry_map"] = qry_map
    example["qry_ex_vocab"] = qry_ex_vocab

    # Very important stage for copy mechanism training.
    if "tgt" in example:
        tgt = tgt_field.tokenize(example["tgt"])

        #Create mask w.r.t source input
        mask_src = torch.LongTensor([unk_idx_src] + [src_ex_vocab.stoi[w] for w in tgt] + [unk_idx_src])
        example["alignment_src"] = mask_src

        #Create mask w.r.t query input
        mask_qry = torch.LongTensor([unk_idx_qry] + [qry_ex_vocab.stoi[w] for w in tgt] + [unk_idx_qry])
        example["alignment_qry"] = mask_qry


    return src_ex_vocab, qry_ex_vocab, example
...
class Dataset(TorchtextDataset):
    ...
    def __init__(self, fields, readers, data, dirs, sort_key,
        ...
        can_copy = ('src_map' in fields and 'alignment_src' in fields) \
                   or ('qry_map' in fields and 'alignment_qry' in fields)
        ...
        for ex_dict in starmap(_join_dicts, zip(*read_iters)):
            ...
            if can_copy:
                ...
                qry_field = fields['qry']
                tgt_field = fields['tgt']

                # this assumes src_field, qry_field and tgt_field are all text
                src_ex_vocab, qry_ex_vocab, ex_dict = _dynamic_dict(
                    ex_dict, src_field.base_field, qry_field.base_field, tgt_field.base_field)
                self.src_vocabs.append(src_ex_vocab)
                self.qry_vocabs.append(qry_ex_vocab)

            ex_fields = {k: [(k, v)] for k, v in fields.items() if
                         k in ex_dict}

            ex = Example.fromdict(ex_dict, ex_fields)      #Usage : Example.class method fromdict(data, fields)
            examples.append(ex)
