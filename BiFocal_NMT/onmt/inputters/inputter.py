...
def make_src(data, vocab):
    src_size = max([t.size(0) for t in data])   #Needs to accomodate the largest seq_len in the batch

    src_vocab_size = max([t.max() for t in data]) + 1    # Needs to accommodate largest vocabulary in the batch
...
def make_tgt(data, vocab):       # Just transposes the input. Nothing special happens here !
...
def get_fields(
    ...
    Args:
        ...
        qry_truncate: Cut off qry sequences beyond this (passed to
            ``qry_data_type``'s data reader - see there for more details).
    ...
    qry_field_kwargs = {"n_feats": n_qry_feats,
                        "include_lengths": True,
                        "pad": pad, "bos": None, "eos": None,
                        "truncate": qry_truncate,
                        "base_name": "qry"}
    fields["qry"] = fields_getters[src_data_type](**qry_field_kwargs)
    ...
    if dynamic_dict:
        ...
        fields["alignment_src"] = align_scr

        qry_map = Field(
            use_vocab=False, dtype=torch.float,
            postprocessing=make_src, sequential=False)
        fields["qry_map"] = qry_map

        qry_ex_vocab = RawField()
        fields["qry_ex_vocab"] = qry_ex_vocab

        align_qry = Field(
            use_vocab=False, dtype=torch.long,
            postprocessing=make_tgt, sequential=False)
        fields["alignment_qry"] = align_qry
...
def load_old_vocab(vocab, data_type="text", dynamic_dict=False):
    ...
    if _old_style_vocab(vocab):
        ...
        n_qry_features = sum('qry_feat_' in k for k in vocab)
        n_tgt_features = sum('tgt_feat_' in k for k in vocab)
        fields = get_fields(
            data_type, n_src_features, n_qry_features, n_tgt_features,
...
def filter_example(ex, use_src_len=True, use_qry_len=True, use_tgt_len=True,
                   min_src_len=1, max_src_len=float('inf'),
                   min_qry_len=1, max_qry_len=float('inf'),
    ...
    Args:
        ...
        use_qry_len (bool): Similar to above.
        ...
        min_qry_len (int): Similar to above.
...
def _build_fields_vocab(fields, counters, data_type, share_vocab,
                        ...
                        qry_vocab_size, qry_words_min_frequency,
                        tgt_vocab_size, tgt_words_min_frequency):
    build_fv_args = defaultdict(dict)
    build_fv_args["src"] = dict(
        max_size=src_vocab_size, min_freq=src_words_min_frequency)
    build_fv_args["qry"] = dict(
        max_size=qry_vocab_size, min_freq=qry_words_min_frequency)
...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
                ...
                qry_vocab_path, qry_vocab_size, qry_words_min_frequency,
    ...
    Args:
        ...
        qry_vocab_path (str): Path to qry vocabulary file.
        qry_vocab_size (int): size of the query vocabulary.
        qry_words_min_frequency (int): the minimum frequency needed to
            include a query word in the vocabulary.
    ...
    if qry_vocab_path:
        qry_vocab, qry_vocab_size = _load_vocab(
            qry_vocab_path, "qry", counters,
            qry_words_min_frequency)
    else:
        qry_vocab = None
