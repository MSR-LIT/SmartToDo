...
class ArgumentParser(cfargparse.ArgumentParser):
    ...
    def validate_preprocess_args(cls, opt):
        ...
        assert len(opt.train_src) == len(opt.train_tgt) == len(opt.train_qry), \
            "Please provide same number of src, qry and tgt train files!"

        assert len(opt.train_src) == len(opt.train_ids), \
            "Please provide proper -train_ids for your data!"

        for file in opt.train_src + opt.train_qry + opt.train_tgt:
        ...
        assert not opt.valid_qry or os.path.isfile(opt.valid_qry), \
            "Please check path of your valid qry file!"
        ...
        assert not opt.qry_vocab or os.path.isfile(opt.qry_vocab), \
            "Please check path of your qry vocab!"
